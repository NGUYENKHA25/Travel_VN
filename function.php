...
function loginFromSocialCallBack($socialUser) {
    include './connect.php';
    $result = mysqli_query($con, "Select `id`,`username`,`email`,`fullname` from `user_data` WHERE `email` ='" . $socialUser['email'] . "'");
    if ($result->num_rows == 0) {
        $result = mysqli_query($con, "INSERT INTO `user_data` (`fullname`,`email`, `status`, `created_time`, `last_updated`) VALUES ('" . $socialUser['name'] . "', '" . $socialUser['email'] . "', 1, " . time() . ", '" . time() . "');");
        if (!$result) {
